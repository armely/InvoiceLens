...
Load-EnvStack -RootPath $repoRoot -FileNames @('.env', '.env.test')

$deploymentName = if ($env:INVOICELENS_INFRA_TEST_DEPLOYMENT_NAME) { $env:INVOICELENS_INFRA_TEST_DEPLOYMENT_NAME } else { 'invoicelens-test-infra' }
$resourcePrefix = if ($env:INVOICELENS_INFRA_RESOURCE_PREFIX) { $env:INVOICELENS_INFRA_RESOURCE_PREFIX } else { 'invoicelens' }
$location = if ($env:INVOICELENS_INFRA_TEST_LOCATION) { $env:INVOICELENS_INFRA_TEST_LOCATION } else { 'eastus2' }
$sqlLocation = if ($env:INVOICELENS_INFRA_TEST_SQL_LOCATION) { $env:INVOICELENS_INFRA_TEST_SQL_LOCATION } else { $location }
